...
app.get('/api/admin/registrations', verifyToken, (req, res) => {
  ...
  try {
    const db = getDb();
    const regs = db.prepare(`SELECT r.*, u.full_name as user_full_name, u.email as user_email FROM registrations r LEFT JOIN users u ON r.user_id = u.id ORDER BY r.submitted_at DESC`).all();
    db.close();
    res.json({ success: true, registrations: regs });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
// GET /api/admin/agreements/all — All agreement signatures (for Concierge tab)
app.get('/api/admin/agreements/all', verifyToken, (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ success: false, message: 'Admin access required.' });
  try {
    const db = getDb();
    db.exec(`CREATE TABLE IF NOT EXISTS agreement_signatures (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER NOT NULL, agreement_type TEXT NOT NULL,
      full_name TEXT NOT NULL, email TEXT NOT NULL,
      signed_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      ip_address TEXT, user_agent TEXT,
      UNIQUE(user_id, agreement_type)
    )`);
    const agreements = db.prepare('SELECT * FROM agreement_signatures ORDER BY signed_at DESC').all();
    db.close();
    res.json({ success: true, agreements });
  } catch(e) { res.status(500).json({ success: false, message: e.message }); }
});

// GET /api/admin/agreements/:userId/:type/pdf — Generate agreement PDF
app.get('/api/admin/agreements/:userId/:type/pdf', verifyToken, (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ success: false, message: 'Admin access required.' });
  const { userId, type } = req.params;
  if (!['service_agreement', 'nda', 'marketing_auth'].includes(type)) {
    return res.status(400).send('Invalid agreement type.');
  }
  try {
    const db = getDb();
    const sig = db.prepare('SELECT * FROM agreement_signatures WHERE user_id = ? AND agreement_type = ?').get(userId, type);
    const user = db.prepare('SELECT email, full_name FROM users WHERE id = ?').get(userId);
    db.close();
    
    if (!sig) return res.status(404).send('Agreement not found.');
    ...
    // Generate simple HTML → browser will offer to save as PDF
...
<div class="seal">
  ...
  <p>For verification, contact: competitions@techinbridge.com</p>
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${type}_agreement_${sig.full_name.replace(/\s/g, '_')}.html"`);
    res.send(html);
  } catch(e) { res.status(500).send('Generation failed: ' + e.message); }
});
